...
def reweight_and_refosm(org_d="model_files"):
    ...
    # the stdev of the ch4 measurements
    ch4_data_std = 3.7

    obs.loc[obs.apply(lambda x: x.obgnme.startswith("ch4_") and x.weight > 0,axis=1),"weight"] = 1.0/ch4_data_std
    obs.loc[obs.apply(lambda x: x.obgnme.startswith("ch4_") and x.weight > 0,axis=1),"standard_deviation"] = ch4_data_std

    # the stdev of the diff measurements
    diff_data_std = 0.87

    obs.loc[obs.apply(lambda x: x.obgnme.startswith("diff_") and x.weight > 0,axis=1),"weight"] = 1.0/diff_data_std
    obs.loc[obs.apply(lambda x: x.obgnme.startswith("diff_") and x.weight > 0,axis=1),"standard_deviation"] = diff_data_std

    # the stdev of the ebb measurements
    ebb_data_std = 4.12

    obs.loc[obs.apply(lambda x: x.obgnme.startswith("ebb_") and x.weight > 0,axis=1),"weight"] = 1.0/ebb_data_std
    obs.loc[obs.apply(lambda x: x.obgnme.startswith("ebb_") and x.weight > 0,axis=1),"standard_deviation"] = ebb_data_std
...
def prep_for_ies(org_d="model_files"):
    ...
    obs.loc[obs.obsnme.str.contains("diff"),"lower_bound"] = 0.0
    obs.loc[obs.obsnme.str.contains("ebb"),"lower_bound"] = 0.0
    obs.loc[obs.obsnme.str.contains("ch4"),"lower_bound"] = 0.0
